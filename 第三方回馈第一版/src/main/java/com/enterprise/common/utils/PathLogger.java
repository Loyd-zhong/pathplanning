...
public class PathLogger {
    ...
    public static void logPath(String agvId, Path path, LocalDateTime startTime, String decision) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(LOG_FILE_PATH, true))) {
            writer.println("=== 新的路径规划记录 ===");
            writer.println("时间：" + LocalDateTime.now().format(formatter));
            writer.println("AGV ID: " + agvId);
            writer.println("开始时间: " + startTime.format(formatter));
            writer.println("决策: " + decision);
